...
<%
	...
	if("login".equals(command)){
		...
		try{
			if(userId2.trim()==null){
				throw new UserNotFoundException("验证码不能为空");
			}
			if(!userId2.equalsIgnoreCase(srand.toLowerCase())){
...
<HTML>
	<head>
		...
		if(window.self!=window.top){
			window.top.location=window.self.top;
		}
		function init(){
		...
		//	var v1='<%=session.getAttribute("rand") %>';
		//	alert(v1+"   "+field.value);
		//	if(field.value.toLowerCase()!=v1.toLocaleLowerCase()){
		//		document.getElementById("spanUserId2").innerHTML="<font color='red'>"+"验证码错误"+"</font>";
		//		return;
		//	}else{
		//		document.getElementById("spanUserId2").innerHTML="<font color='red'>"+"验证码正确"+"</font>";
		//	}
