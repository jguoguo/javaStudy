...
public class FlowCardDaoImpl implements FlowCardDao {
	...
	public void delFlowCardMaster(String[] flowCardVouNos) throws DaoException {
		...
		String sql="delete from t_flow_card_master where flow_card_no in ("+sbStr.toString()+")";
		PreparedStatement pstmt=null;
		try{
			Connection conn=ConnectionManager.getConnection();
			pstmt=conn.prepareStatement(sql);
			...
			e.printStackTrace();
			System.out.println("FlowCardImpl-->delFlowCardDetail,exception"+e);
			throw new DaoException(e);
		}finally{
			ConnectionManager.close(pstmt);
		}
	...
	public List<FlowCard> findFlowCardList(int pageNo, int pageSize,
		...
		.append("( ")
		  ...
		  .append("from( ")
		  .append("select a.flow_card_no,b.client_id,b.name as client_name,c.user_name as user_name,a.opt_date ")
	...
	public int getRecordCount(String clientId, Date beginDate, Date endDate)
			throws DaoException {
		String sql="select count(*) from t_flow_card_master a join t_client b on a.client_id=b.id join t_user c on a.recorder_id=c.user_id where a.vou_sts='N' and";
		if(clientId !=null && !"".equals(clientId)){
			sql+=" b.client_id=? and ";
		}
	...
	public void auditFlowCard(String[] flowCardVouNos) throws DaoException {
		...
		try{
			...
			e.printStackTrace();
			System.out.println("FlowCardImpl-->auditFlowCard,exception"+e);
			throw new DaoException(e);
		}finally{
			ConnectionManager.close(pstmt);
		}
